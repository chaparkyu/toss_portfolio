...
st.write("스마트폰, 태블릿, PC 어디서든 사용할 수 있는 웹 계산기입니다.")
st.divider()

# 4. 입력 폼 구성 (스트림릿은 콤마 처리가 자동입니다!)
        ...
        if not error_flag:
            ...
            for name in PORTFOLIO_TICKERS:
                ...
                with st.container(border=True):
                    st.markdown(f"**[{name}]**")
                    st.write(f"- 현재가: {price:,.0f}원 | 보유: {held_shares:,.0f}주 | 평가금: {current_equity:,.0f}원")
                    ...
                    if shortfall <= 0:
                        ...
                        st.warning(f"👉 **{shares_to_buy:,.0f}주 추가 매수** (예상 비용: {actual_cost:,.0f}원)")
